...
CLASSIFICATION_MAX_BATCH = int(os.getenv("CLASSIFICATION_MAX_BATCH", 16))
...
DEDUPE_EMBEDDING_NUM_PROCESSES_PER_GPU = int(os.getenv("DEDUPE_EMBEDDING_NUM_PROCESSES_PER_GPU", 2))
"""Number of processes to run in parallel per GPU"""

DEDUPE_EMBEDDING_BATCH_SIZE = int(os.getenv("DEDUPE_EMBEDDING_BATCH_SIZE", 256))
...
DEDUPE_EMBEDDING_MAX_NEIGHBORS = int(os.getenv("DEDUPE_EMBEDDING_MAX_NEIGHBORS", 30))
"""Maximum neighbors to find per embedding (k in HNSW search)"""

DEDUPE_EMBEDDING_MAX_CONNECTIONS = int(os.getenv("DEDUPE_EMBEDDING_MAX_CONNECTIONS", 12))
"""HNSW index M parameter (number of connections per layer)"""

DEDUPE_EMBEDDING_HNSW_EF_CONST = int(os.getenv("DEDUPE_EMBEDDING_HNSW_EF_CONST", 100))
"""HNSW ef_construction parameter (index build time, higher=better recall)"""

DEDUPE_EMBEDDING_HNSW_EF_SEARCH = int(os.getenv("DEDUPE_EMBEDDING_HNSW_EF_SEARCH", 30))
"""HNSW ef_search parameter (query time, higher=better recall)"""

DEDUPE_EMBEDDING_HNSW_INDEX_BATCH = int(os.getenv("DEDUPE_EMBEDDING_HNSW_INDEX_BATCH", 100000))
"""Batch size for index building"""

DEDUPE_EMBEDDING_SEARCH_BATCH = int(os.getenv("DEDUPE_EMBEDDING_SEARCH_BATCH", 50000))
...
"""Size of phash (hash_size parameter). The hash will be of size HASH_SIZE*HASH_SIZE bits (144 bits total)."""

HASH_DB_CHUNK_SIZE = int(os.getenv("HASH_DB_CHUNK_SIZE", 10000))
...
HASH_DEDUPE_CPUS_LIMIT = min(int(os.getenv("HASH_DEDUPE_CPUS_LIMIT", 64)), CPUS_LIMIT)
"""Limit the number of CPUs for hash deduping. Capped at CPUS_LIMIT."""
...
# Captioning
#
CAPTION_MAX_IMG_DIM = int(os.getenv("CAPTION_MAX_IMG_DIM", 1248))
""" Max dimension to send to OpenAI (pixels)"""

CAPTION_MAX_TOKENS = int(os.getenv("CAPTION_MAX_TOKENS", 100))
""" Max tokens for model to produce"""

CAPTION_MODEL_NAME = os.getenv("CAPTION_MODEL_NAME", "gpt-4.1-nano")
""" Model to generate captions"""

CAPTION_MODEL_ENDPOINT = os.getenv("CAPTION_MODEL_ENDPOINT", None)
""" Optional custom endpoint for the captioning model. When set, overrides the default OpenAI base URL. """

CAPTION_MODEL_TEMPERATURE = float(os.getenv("CAPTION_MODEL_TEMPERATURE", 0))
...
OPENAI_REQUEST_TIMEOUT = float(os.getenv("OPENAI_REQUEST_TIMEOUT", 20.0))
"""Number of seconds to wait before API request timeout"""

CAPTION_REQUEST_RETRY_ATTEMPTS = int(os.getenv("CAPTION_REQUEST_RETRY_ATTEMPTS", 2))
...
CAPTION_MAX_BATCH_SIZE = int(os.getenv("CAPTION_MAX_BATCH_SIZE", 8))
"""Max request batch size to send at once to OpenAI"""

CPUS_LIMIT_CAPTION = int(os.getenv("CPUS_LIMIT_CAPTION", 20))
...
COUNT_TOKENS_SERVER_SIDE_CURSOR_SIZE = int(os.getenv("COUNT_TOKENS_SERVER_SIDE_CURSOR_SIZE", 100_000))
""" Number of rows to fetch per batch when streaming results via PostgreSQL server-side cursors. """

COUNT_TOKENS_WORKERS = int(os.getenv("COUNT_TOKENS_WORKERS", 4))
""" Number of parallel workers for the count-tokens step. """

HF_THESAURUS_REPO = "institutional/chronicling-america-thesauri"
""" Hugging Face repo containing thesaurus data. """

BACKFILL_DEFAULT_WORKERS = int(os.getenv("BACKFILL_DEFAULT_WORKERS", 4))
""" Number of parallel workers for the backfill step. """

BACKFILL_VACUUM_EVERY_N_CHUNKS = int(os.getenv("BACKFILL_VACUUM_EVERY_N_CHUNKS", 100))
""" Run VACUUM after this many chunks during backfill to reclaim space. """

EMBEDDING_ATLAS_SAMPLE = int(os.getenv("EMBEDDING_ATLAS_SAMPLE", 10000))
""" Number of embeddings to sample for atlas visualization. """

EMBEDDING_ATLAS_PORT = int(os.getenv("EMBEDDING_ATLAS_PORT", 5055))
""" Port for the embedding atlas visualization server. """

POST_PROCESS_WORKERS = int(os.getenv("POST_PROCESS_WORKERS", 4))
""" Number of parallel workers for post-processing steps. """
...
S3_EXPORT_ROW_GROUP_SIZE = int(os.getenv("S3_EXPORT_ROW_GROUP_SIZE", 500))
""" Number of records per Parquet row group. Kept small to limit memory from crop bytes. """

S3_EXPORT_MULTIPART_THRESHOLD = 100 * 1024 * 1024  # 100 MB
""" File size above which S3 uploads switch to multipart. """

S3_EXPORT_MULTIPART_CHUNK_SIZE = 50 * 1024 * 1024  # 50 MB
""" Size of each part in a multipart S3 upload. """

S3_EXPORT_MULTIPART_PARALLEL_PARTS = int(os.getenv("S3_EXPORT_MULTIPART_PARALLEL_PARTS", 6))
""" Number of parts to upload to S3 concurrently during a multipart S3 upload. """

S3_EXPORT_BATCH_SIZE_DB = int(os.getenv("S3_EXPORT_BATCH_SIZE_DB", 500_000))
""" Batch size for PostgreSQL IN-clause queries to avoid exceeding memory limits. """

S3_EXPORT_SAMPLE_LIMIT = 10_000
""" Maximum number of records to export to S3 when running in sample mode. """

S3_EXPORT_MAX_PENDING_UPLOADS = int(os.getenv("S3_EXPORT_MAX_PENDING_UPLOADS", 8))
""" Maximum number of concurrent shard uploads to S3 before backpressure pauses record processing. """

S3_EXPORT_MAX_INFLIGHT = int(os.getenv("S3_EXPORT_MAX_INFLIGHT", 4))
""" Maximum number of S3 item download tasks in flight at once during export. """

S3_EXPORT_SHARD_SIZE = int(os.getenv("S3_EXPORT_SHARD_SIZE", 5000))
""" Default number of rows per Parquet shard file uploaded to S3. """

S3_ITEMS_PER_FETCH = int(os.getenv("S3_ITEMS_PER_FETCH", 50))
""" Number of S3 items to fetch per request during export. """
...
HF_EXPORT_SHARD_SIZE = int(os.getenv("HF_EXPORT_SHARD_SIZE", 5000))
""" Number of rows per parquet shard uploaded to the HF dataset repo. """

HF_EXPORT_IMAGE_BATCH_SIZE = int(os.getenv("HF_EXPORT_IMAGE_BATCH_SIZE", 1000))
""" Number of images to upload per commit to the HF images bucket. """

HF_EXPORT_ITEMS_PER_FETCH = int(os.getenv("HF_EXPORT_ITEMS_PER_FETCH", 200))
""" Number of pipeline batch items to fetch from the database per query. """

HF_EXPORT_IO_WORKERS = int(os.getenv("HF_EXPORT_IO_WORKERS", 4))
...
DEFAULT_DB_BATCH_SIZE = int(os.getenv("DEFAULT_DB_BATCH_SIZE", 1000))
"""
...
MAX_S3_REQUESTS_PER_SECOND = int(os.getenv("MAX_S3_REQUESTS_PER_SECOND", 25))

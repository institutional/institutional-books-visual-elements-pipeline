...
def _apply_orientation_correction(png_bytes: bytes, correction: str | None) -> bytes:
    if not correction or correction == "upright":
        return png_bytes
    rotation = CV2_ROTATION_MAP.get(correction)
    if rotation is None:
        return png_bytes
    arr = cv2.imdecode(np.frombuffer(png_bytes, np.uint8), cv2.IMREAD_COLOR)
    if arr is None:
        return png_bytes
    rotated = cv2.rotate(arr, rotation)
            ...
            for batch_start in range(0, len(item_work), io_workers):
                ...
                for fut in as_completed(futures):
                    ...
                    for r in processed:
                        ...
                        crop_bytes = _apply_orientation_correction(crop_bytes, r["orientation_correction"])
